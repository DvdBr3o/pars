...
struct DynamicError {
	...
	template<typename ErrorT>
		requires requires(const std::remove_cvref_t<ErrorT>& error) {
			{ pars::to_string(error) } -> std::convertible_to<std::string>;
		}
	constexpr DynamicError(ErrorT&& error) :
		message(pars::to_string(std::forward<ErrorT>(error))) {}
...
inline constexpr auto tag_invoke(to_string_t, const std::variant<Es...>& es) {
	return std::visit([]<typename E>(const E& e) -> std::string { return pars::to_string(e); }, es);
}
...
inline auto token_debug_string(const T& token) -> std::string {
	...
	else if constexpr (requires { pars::to_string(token); })
		return pars::to_string(token);
...
private:
	...
	static_assert(
		std::same_as<InnerValueType, ValueT> || std::same_as<InnerValueType, std::unique_ptr<ValueT>>,
...
struct PolymorphicParserStateView {
	...
	constexpr explicit PolymorphicParserStateView(QueryableAll<RequiredQueryTs...> auto&& st) :
		vtable {
			QueryableSlot<RequiredQueryTs> {std::addressof(pars::query<RequiredQueryTs>(st))}...
		} {}
...
// template<typename... RuleTLs, typename RuleTR>
// inline constexpr auto operator>>(SequentialRule<RuleTLs...>&& l, RuleTR&& r)
// 	-> SequentialRule<RuleTLs..., std::remove_cvref_t<RuleTR>> {
// 	return std::apply(
// 		[&]<typename... RuleTs>(RuleTs&&... rules)
// 			-> SequentialRule<RuleTLs..., std::remove_cvref_t<RuleTR>> {
// 			return {std::forward<RuleTs>(rules)..., std::forward<RuleTR>(r)};
// 		},
// 		std::move(l.rules)
// 	);
// }

// template<typename... RuleTRs, typename RuleTR>
// inline constexpr auto operator>>(RuleTR&& l, SequentialRule<RuleTRs...>&& r)
// 	-> SequentialRule<std::remove_cvref_t<RuleTR>, RuleTRs...> {
// 	return r.rules
// 		 | apply(
// 			   [&]<typename... RuleTs>(
// 				   RuleTs&&... rules
// 			   ) -> SequentialRule<std::remove_cvref_t<RuleTR>, RuleTRs...> {
// 				   return {l, std::forward<RuleTs>(rules)...};
// 			   }
// 		 );
// }
...
// template<typename... RuleTLs, typename... RuleTRs>
// inline constexpr auto operator>>(SequentialRule<RuleTLs...>&& l, SequentialRule<RuleTRs...>&& r)
// 	-> SequentialRule<RuleTLs..., RuleTRs...> {
// 	return l | apply([&]<typename... InternalRuleTLs>(InternalRuleTLs&&... lrules) {
// 			   return r
// 					| apply(
// 						  [&]<typename... InternalRuleTRs>(
// 							  InternalRuleTRs&&... rrules
// 						  ) -> SequentialRule<RuleTLs..., RuleTRs...> {
// 							  return {
// 								  std::forward<InternalRuleTLs>(lrules)...,
// 								  std::forward<InternalRuleTRs>(rrules)...,
// 							  };
// 						  }
// 					);
// 		   });
