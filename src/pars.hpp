...
struct ParserKit {
	...
	private:
		[[nodiscard]] auto _char_match(char32_t ch) const -> bool { return _char_match(ch, 0, N); }
	...
	struct OnceOrMoreRule : R {
		...
		constexpr OnceOrMoreRule(auto&&... args) : R { std::forward<decltype(args)>(args)... } {}

		auto match(ParserStateT& ps) const -> tl::expected<Value, Error> {
			Value val;
			auto  restore	  = ps.store();
	...
	struct RepeatableRule : R {
		...
		constexpr RepeatableRule(auto&&... args) : R { std::forward<decltype(args)>(args)... } {}
